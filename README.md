...
`temperature: 0` · `SHA-256 receipt chain` · `fails closed` · `50 tests`

[Quick Start](#quick-start) · [How It Works](#how-it-works) · [CLI Reference](#cli-reference) · [Rules Format](#rules-format) · [CI Integration](#ci-integration) · [Architecture](#architecture) · [API Reference](#api-reference)
...
FORT:
  ...
  rules:
    ...
    - id: FORT-003
      name: No bare except
      description: "except clauses must specify an exception type — bare except: is forbidden"
      severity: medium
    - id: FORT-004
...
Some rules are strict by design and may produce noise in codebases that don't follow the convention they enforce:
...
The repo ships with a domain-specific extension for multi-tenant legal SaaS environments:

```bash
python audit_agent.py --rules rules/legal-saas.yaml src/
```

`rules/legal-saas.yaml` adds 10 rules covering dynamic code evaluation on tenant data, cross-tenant data paths, unvalidated document input, audit logging requirements on mutations, and external call isolation. See the file for full definitions.

**Writing custom rules**

Create a new YAML file following the schema above. Rules are injected verbatim into the audit prompt, so write descriptions that are specific and unambiguous:
...
```json
{
  ...
  "result": {
    ...
    "summary": "eval() call on line 42 violates SPOT-001. Dynamic code execution on user-supplied input is forbidden.",
    "spot_violations": [
      {
        "rule_id": "SPOT-001",
        "rule_name": "No eval/exec",
        "severity": "critical",
        "location": "line 42",
        "detail": "eval(user_input) — evaluates unsanitized user input as Python code"
      }
    ],
    "fort_violations": [],
    "retry_count": 0
  ...
  "chain": {
    "previous_receipt_hash": "f0a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1"
  },
  "receipt_hash": "f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8"
}
```
...
{
  "version": "2.0",
  "timestamp": "2026-02-22T14:30:04.000000+00:00",
  "file": {
    "path": "utils.py",
  ...
  "chain": {
    "previous_receipt_hash": "f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8"
  },
  "receipt_hash": "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2"
}
```
...
**What enforcement mode looks like in a CI log**

```
Auditing src/auth.py...
  CRITICAL [SPOT-001] No eval/exec — line 42: eval(user_input)
Auditing src/utils.py...
  CLEAN
Auditing src/models.py...
  MEDIUM [FORT-001] Function complexity — process_records() exceeds 50 statements (67)

Results: 2 violations across 3 files
Receipts written to: auth.py.receipt.json, utils.py.receipt.json, models.py.receipt.json
Exit code: 1
```

The pipeline blocks. Receipts are uploaded as artifacts regardless of pass/fail (`if: always()`).
...
**Prompt determinism.** `build_prompt(code, filename, rules_text)` is a pure function. Same inputs, same output. No timestamps, no random IDs in the prompt.

**Rules are external YAML.** No rules are hardcoded. Swap per-repo or per-team.

**Receipts are hash-chained.** Each receipt includes SHA-256 hashes of the file, rules, and previous receipt. Verifiable offline without re-running the audit.

**Claude is the reasoning engine, not the policy store.** Rules are injected verbatim. Claude evaluates code against those rules and returns structured JSON.

**Fails closed.** Invalid schema output after 3 retries raises `GovernanceValidationError`. API timeout after 30 seconds raises `RuntimeError`. Missing API key raises `EnvironmentError`. No silent degradation.

**Zero framework dependencies.** `requests` and `pyyaml`. No agent orchestration libraries, no ORMs, no DSLs.
...
Load and validate a YAML rules file. Raises `FileNotFoundError` if the file doesn't exist. Raises `ValueError` if the schema is invalid.
...
m87-audit-agent/
  ...
  rules/
    legal-saas.yaml          # Domain-specific rules for multi-tenant legal SaaS
  tests/
    test_audit_agent.py      # Full test suite (50 tests)
...
4. Run `python audit_agent.py src/` in audit-only mode first. Review the receipts to establish a baseline before switching to enforcement. This surfaces existing violations so you can triage them before they start blocking PRs.
5. Add the GitHub Actions workflow above to `.github/workflows/audit.yml`.
6. Switch to `--fail-on-violations` on PRs to `main` once the baseline is clean.
